...
const CatalogoMujer = () => {
  ...
  useEffect(() => {
    homeService.getCatalogoMujer().then(data => {
  ...
  const labels = {
    titulo: language === 'es' ? 'Botas para Dama' : "Women's Boots",
    subtitulo: language === 'es' ? 'Elegancia y distinción en cada paso, diseñadas para la mujer contemporánea' : 'Elegance and distinction in every step, designed for the contemporary woman',
            ...
            <div className="max-w-6xl mx-auto flex flex-col sm:flex-row flex-wrap gap-4 items-center justify-between">
              <div>
                <p className="text-caborca-cafe font-semibold text-sm sm:text-base">
                  <span id="productCount">{listaProductos.length}</span> {labels.encontrados}
                </p>
              </div>
